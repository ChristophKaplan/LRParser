...
public class LRParser {
    ...
    public LRParser(ContextFreeGrammar cfg) {
        this.cfg = cfg;
            ...
            if (cur.GetSymbol() is NonTerminal nonTerminal) {
                ...
                foreach (var prod in prods) {
                    ...
                    if (containedAlready.Count == 0) {
                        ...
                        if(containedAlready.Count > 1) throw new Exception("should not exist more than one time" + containedAlready.Aggregate("", (c, n) => $"{c} {n}, "));
    ...
    private List<State> GenerateStates( LRItem startItem) {
        ...
        int count = 0;
        EnfoldTransitions(firstState,states,ref count);
        return states;
    }
    
    private void EnfoldTransitions(State current, List<State> states, ref int count) {
        ...
        foreach (var possibleTransitions in possibleTransitionGroups) {
            ...
            if (possibleTransitions.Key.Equals(new Terminal())) continue;
            ...
            if(TryGetSameState(states, next, out var sameState)) {
                ...
                states.Add(next);
                EnfoldTransitions(next,states,ref count);
            }
        }
    }
